            ...
            case LLM_ARCH_ERNIE4_5_VL_MOE:
            {
                ...
                for (int i = 0; i < n_layer; ++i) {
                    ...
                    if (static_cast<uint32_t>(i) >= hparams.n_layer_dense_lead) { // MoE layers
                        ...
                        layer.v_ffn_gate_exps = create_tensor(tn(LLM_TENSOR_V_FFN_GATE_EXPS, "weight", i), {n_embd,   512, n_expert}, TENSOR_NOT_REQUIRED);
                        layer.v_ffn_down_exps = create_tensor(tn(LLM_TENSOR_V_FFN_DOWN_EXPS, "weight", i), {  512, n_embd, n_expert}, 0);
                        layer.v_ffn_up_exps   = create_tensor(tn(LLM_TENSOR_V_FFN_UP_EXPS,   "weight", i), {n_embd,   512, n_expert}, 0);
